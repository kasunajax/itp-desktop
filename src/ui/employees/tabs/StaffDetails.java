...
public class StaffDetails extends KTab {
	...
	public void Create() {
		...
		/*
		 * String EmpID = txtempid.getSelectedText(); String FName =
		 * txtfname.getSelectedText(); String LName = txtlname.getSelectedText(); String
		 * Gender = cmbgender.getSelectedItem().toString(); //String Gender = "MALE"; //
		 * Date DOB = Date.valueOf(dob.getDate().toString()); // Date DOB=new
		 * Date(dob.getDate().getTime()); String NIC = txtnic.getSelectedText(); String
		 * Address = txthaddress.getSelectedText(); String mobno =
		 * txtmobno.getSelectedText(); String fixedno = txtfixedno.getSelectedText();
		 * String Designation = txtdesignation.getSelectedText(); String UName =
		 * txtuname.getSelectedText(); String PWord = txtpword.getSelectedText(); //
		 * String Type = cmbtype.getSelectedItem().toString(); String Type =
		 * "INTERNAL STAFF"; String IVR = txtivrno.getSelectedText(); // Date Sdate =
		 * Date.valueOf(SDate.getDate().toString()); // Date Edate =
		 * Date.valueOf(EDate.getDate().toString()); String PayrollID =
		 * txtpayrollid.getSelectedText(); String OfficeID =
		 * txtofficeid.getSelectedText();
		 */
		/*
		 * String EmpID = txtempid.getText(); String FName = txtfname.getText(); String
		 * LName = txtlname.getText(); String NIC = txtnic.getText(); String Address =
		 * txthaddress.getText(); String Designation = txtdesignation.getText(); String
		 * UName = txtuname.getText(); String PWord = txtpword.getText(); String
		 * PayrollID = txtpayrollid.getText(); String OfficeID = txtofficeid.getText();
		 * String MobNo = txtmobno.getText(); String FixedNo = txtfixedno.getText();
		 */

		/*
		 * Pattern pattern1 = Pattern.compile("\\d{10}"); Matcher mobile =
		 * pattern1.matcher(mobno);
		 * 
		 * Pattern pattern2 = Pattern.compile("\\d{10}"); Matcher fixed =
		 * pattern2.matcher(fixedno);
		 * 
		 * Pattern pattern3 = Pattern.compile("\\d{9}\\p{Upper}}");
		 * 
		 * if (FName.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter First Name"); }
		 * 
		 * else if (LName.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter Last Name"); }
		 * 
		 * else if (Gender.equals("GENDER")) { JOptionPane.showMessageDialog(null,
		 * "Please select a gender"); } else if (DOB.equals("")) {
		 * JOptionPane.showMessageDialog(null, "That is not a valid date"); } /* else
		 * if(!nic.matches() || NIC.equals("")){
		 * JOptionPane.showMessageDialog(null,"NIC"); }
		 */
		/*
		 * else if (!mobile.matches() || mobile.equals("")) {
		 * JOptionPane.showMessageDialog(null, "Please enter a valid Mobile no"); } else
		 * if (!fixed.matches() || fixed.equals("")) {
		 * JOptionPane.showMessageDialog(null, "Please enter a valid Fixed Line no"); }
		 * else if (Address.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter Address"); } else if (Type.equals("SELECT TYPE")) {
		 * JOptionPane.showMessageDialog(null, "Please select the type of employee"); }
		 * else if (EmpID.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter EmployeeID"); } else if (OfficeID.equals("")) {
		 * JOptionPane.showMessageDialog(null, "Enter OfficeID"); } else if
		 * (PayrollID.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter PayrollID"); } else if (Designation.equals("")) {
		 * JOptionPane.showMessageDialog(null, "Enter Designation"); } else if
		 * (UName.equals("")) { JOptionPane.showMessageDialog(null, "Enter UserName"); }
		 * else if (PWord.equals("")) { JOptionPane.showMessageDialog(null,
		 * "Enter Password"); } else if (Sdate.equals("")) {
